...
resource "aws_instance" "elasticsearch_node" {
  ...
  associate_public_ip_address = true
  subnet_id                   = aws_subnet.custom-public-subnet.id
  vpc_security_group_ids      = [aws_security_group.allow_ssh.id, aws_security_group.allow_elasticsearch_port.id, aws_security_group.allow_kibana_custom_http_port.id]
  key_name                    = module.key_pair.key_pair_key_name
  tags                        = {
    Name = "elasticsearch"
  }
